"""Loads environment-driven configuration for Google ADK, optional OpenAI helpers, Deepgram, ElevenLabs, CORS, models, sample rates, and session persistence."""
...
class Settings(BaseSettings):
    ...
    backend_port: int = Field(default=8001, alias="BACKEND_ADK_PORT")
    backend_base_url: str = Field(default="http://127.0.0.1:8001", alias="BACKEND_ADK_BASE_URL")
    ...
    callcenter_session_db_path: str = Field(
        default="backend_adk/.data/callcenter_sessions.db",
        alias="CALLCENTER_SESSION_DB_PATH",
    )
    adk_session_db_path: str = Field(
        default="backend_adk/.data/callcenter_sessions.db",
        alias="ADK_SESSION_DB_PATH",
    )
